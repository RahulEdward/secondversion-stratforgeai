...
Adding a new tool:
  1. Create a file in app/agent_tools/ with a class extending BaseTool
...
def _discover_subclasses() -> list[type[BaseTool]]:
    ...
    for _, module_name, _ in pkgutil.iter_modules([pkg_dir]):
        if module_name.startswith("_"):
            continue
        try:
            importlib.import_module(f"app.agent_tools.{module_name}")
        except Exception as exc:
            logger.warning("Skipped app.agent_tools.%s: %s", module_name, exc)
...
def build_registry(
    *,
    ...
    """
    from app.agent_tools.remember_tool import RememberTool
    from app.agent_tools.swarm_tool import SwarmTool
