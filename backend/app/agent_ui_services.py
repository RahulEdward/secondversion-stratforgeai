...
def reconstruct_price_series(run_dir: Path) -> List[Dict[str, Any]]:
    ...
    # The original Vibe-Trading ui_services probed ``.env`` via
    # ``src.providers.llm._ensure_dotenv``. StratForge manages credentials in
    # the OS keyring, so we simply no-op that lookup here.
    try:
    ...
    try:
        ...
        if source == "okx":
            from app.backtest_engine.loaders.okx import DataLoader
        elif source == "yfinance":
            from app.backtest_engine.loaders.yfinance_loader import DataLoader
        else:
            from app.backtest_engine.loaders.tushare import DataLoader
        loader = DataLoader()
